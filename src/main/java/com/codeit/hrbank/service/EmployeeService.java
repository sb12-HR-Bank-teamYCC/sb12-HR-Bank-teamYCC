...
public class EmployeeService {
    ...
    public EmployeeDto create(EmployeeCreateRequest request,
        ...
        // 프로필 이미지를 uploads/images/에 저장
        FileMetadata profileImage = fileMetadataService.saveProfileImage(profile);
        ...
        changeLogService.logCreated(savedEmployee, request.memo(), ipAddress);
    ...
    public EmployeeDto update(UUID id, EmployeeUpdateRequest request,
        ...
        // 새 프로필 이미지가 전달된 경우에만 uploads/images/에 저장
        if (profile != null && !profile.isEmpty()) {
            newProfileImage = fileMetadataService.saveProfileImage(profile);
        }
    ...
    public List<EmployeeTrendDto> getTrend(LocalDate from, LocalDate to, String unit) {
        ...
        LocalDate startBucket = from == null
            ? calculateDefaultFrom(endBucket, normalizedUnit)
        ...
        for (LocalDate bucket : buckets) {
            ...
            for (LocalDate hireDate : hireDates) {
                if (!hireDate.isAfter(bucketEnd)) count++;
            }
    ...
    public CursorPageResponse<EmployeeDto> getEmployees(
        ...
        if (hasNext) employees = employees.subList(0, size);
